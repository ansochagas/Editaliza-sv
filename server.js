...
if (process.env.SESSION_SECRET_FILE) {
  process.env.SESSION_SECRET = fs
    .readFileSync(process.env.SESSION_SECRET_FILE, "utf8")
    .trim();
}
...
const safeRollback = async (context = "UNKNOWN") => {
  try {
    ...
    if (!rollbackError.message.includes("no transaction is active")) {
      console.error(
        `[${context}] Erro inesperado ao fazer rollback:`,
...
app.get("/plans", authenticateToken, async (req, res) => {
  try {
    ...
    res.json(
      rows.map((plan) => ({
        ...plan,
        study_hours_per_day: JSON.parse(plan.study_hours_per_day || "{}"),
      }))
    );
  } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (!row)
        return res
          .status(404)
          .json({ error: "Plano não encontrado ou não autorizado." });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!plan)
        return res
          .status(404)
          .json({ error: "Plano não encontrado ou você não tem permissão." });

      await dbRun("BEGIN TRANSACTION");
      await dbRun("DELETE FROM study_sessions WHERE study_plan_id = ?", [
        planId,
      ]);
      await dbRun(
      ...
      res.json({
        message: "Plano e todos os dados associados foram apagados com sucesso",
      });
    } catch (error) {
      await dbRun("ROLLBACK");
...
app.patch(
  ...
  async (req, res) => {
    ...
    const sql =
      "UPDATE study_plans SET daily_question_goal = ?, weekly_question_goal = ?, review_mode = ?, session_duration_minutes = ?, study_hours_per_day = ?, has_essay = ?, reta_final_mode = ? WHERE id = ? AND user_id = ?";
    try {
      const result = await dbRun(sql, [
        daily_question_goal,
        weekly_question_goal,
...
app.post(
  "/plans/:planId/subjects_with_topics",
  authenticateToken,
  validators.numericId("planId"),
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (topics.length > 0) {
        const insertTopicsStmt = db.prepare(
          "INSERT INTO topics (subject_id, description) VALUES (?,?)"
        );
        topics.forEach((topic) =>
          insertTopicsStmt.run(subjectId, topic.substring(0, 500))
        );
        await new Promise((resolve, reject) =>
          insertTopicsStmt.finalize((err) => (err ? reject(err) : resolve()))
        );
      }
      ...
      res
        .status(201)
        .json({ message: "Disciplina e tópicos adicionados com sucesso!" });
    } catch (error) {
      await dbRun("ROLLBACK");
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (result.changes === 0)
        return res
          .status(404)
          .json({ error: "Disciplina não encontrada ou não autorizada." });
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await dbRun("COMMIT");
      res.json({
        message: "Disciplina e todos os seus dados foram apagados com sucesso",
      });
    } catch (error) {
      await dbRun("ROLLBACK");
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const rows = await dbAll(
        "SELECT id, description, status, completion_date FROM topics WHERE subject_id = ? ORDER BY id ASC",
        [req.params.subjectId]
      );
      res.json(rows);
    } catch (error) {
    ...
    try {
      ...
      for (const topic of topics) {
        const completionDate =
          topic.status === "Concluído" ? topic.completion_date : null;
        await new Promise((resolve, reject) =>
          stmt.run(topic.status, completionDate, topic.id, req.user.id, (err) =>
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (result.changes === 0)
        return res
          .status(404)
          .json({ error: "Tópico não encontrado ou não autorizado." });
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await dbRun("COMMIT");
      res.json({
        message: "Tópico e sessões associadas foram apagados com sucesso",
      });
    } catch (error) {
      await dbRun("ROLLBACK");
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const plan = await dbGet("SELECT * FROM study_plans WHERE id = ?", [
        planId,
      ]);
      if (!plan) {
        await dbRun("ROLLBACK");
      ...
      const totalWeeklyHours = Object.values(study_hours_per_day).reduce(
        (sum, h) => sum + (parseInt(h, 10) || 0),
        0
      );
      if (totalWeeklyHours === 0) {
        ...
        return res.status(400).json({
          error:
            "O cronograma não pode ser gerado porque não há horas de estudo definidas.",
        });
      }
      ...
      if (allTopics.length === 0) {
        await dbRun("COMMIT");
        return res.json({
          message: "Nenhum tópico encontrado para gerar o cronograma.",
        });
      }
      ...
      const getAvailableDates = (startDate, endDate, weekdayOnly = false) => {
        ...
        while (currentDate <= endDate) {
          ...
          const shouldSkip =
            weekdayOnly && (dayOfWeek === 0 || dayOfWeek === 6);

          if (!shouldSkip && (study_hours_per_day[dayOfWeek] || 0) > 0) {
            dates.push({
              date: new Date(currentDate),
              dayOfWeek,
              maxSessions: Math.floor(
                (study_hours_per_day[dayOfWeek] * 60) / sessionDuration
              ),
            });
          }
      ...
      if (plan.has_essay) {
        const sundayDates = getAvailableDates(today, examDate).filter(
          (d) => d.dayOfWeek === 0
        );
        for (const dateInfo of sundayDates) {
          addSessionToAgenda(dateInfo.date, {
            ...
            subjectName: "Redação",
            topicDescription:
              "Prática de redação dissertativa-argumentativa, focando em estrutura, coesão e argumentação.",
            sessionType: "Redação",
          });
        }
      }
      ...
      const getNextSaturdayForReview = (date) => {
        const saturdayDates = getAvailableDates(date, examDate).filter(
          (d) => d.dayOfWeek === 6
        );
      ...
      const availableSlots = getAvailableDates(today, examDate, true).reduce(
        (sum, d) => sum + d.maxSessions,
        0
      );
      ...
      if (pendingTopics.length > availableSlots) {
        ...
        topicsToSchedule.forEach((t) => {
          if (!subjectsMap.has(t.subject_name)) {
            subjectsMap.set(t.subject_name, {
              name: t.subject_name,
              weight: t.priority,
            });
          }
      ...
      const weightedTopics = topicsToSchedule.flatMap((t) =>
        Array(t.priority).fill(t)
      );
      for (let i = weightedTopics.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [weightedTopics[i], weightedTopics[j]] = [
          weightedTopics[j],
          weightedTopics[i],
        ];
      }
      const uniquePendingTopicsInOrder = [
        ...new Map(weightedTopics.map((item) => [item.id, item])).values(),
      ];
      ...
      let maintenanceStartDate = lastNewTopicDate
        ? new Date(lastNewTopicDate)
        : new Date(today);
      ...
      if (!hasPendingNewTopics) {
        console.log(
          `[CRONOGRAMA] Todas as matérias foram cobertas. Iniciando fase de simulados...`
        );
        ...
        if (pendingTopics.length === 0 && progressPercentage >= 0.95) {
          ...
          allTopics.forEach((topic) => {
            if (!subjectCompletionMap.has(topic.subject_name)) {
              subjectCompletionMap.set(topic.subject_name, {
                completed: [],
                total: 0,
              });
            }
          ...
          const subjectsReadyForSim = Array.from(subjectCompletionMap.entries())
            ...
            .sort(
              ([, dataA], [, dataB]) =>
                dataB.completed.length - dataA.completed.length
            );
          ...
          for (const [subjectName, subjectData] of subjectsReadyForSim) {
            ...
            for (
              let groupIndex = 0;
              groupIndex < topicGroups.length;
              groupIndex++
            ) {
              const topicGroup = topicGroups[groupIndex];
              const nextSimulatedDay = findNextAvailableSlot(
                currentDirectedSimDate,
                false
              );
              if (!nextSimulatedDay) break;

              const groupNumber =
                topicGroups.length > 1 ? ` - Bloco ${groupIndex + 1}` : "";
              const topicList = topicGroup
                .map((topic) => `• ${topic}`)
                .join("\n");
              ...
              currentDirectedSimDate.setDate(
                currentDirectedSimDate.getDate() + 3
              );
            }
          }
        }
        ...
        if (pendingTopics.length === 0 && progressPercentage >= 0.95) {
          ...
          const basicCompleteSlot = findNextAvailableSlot(
            basicCompleteSimDate,
            false
          );
          if (basicCompleteSlot) {
            addSessionToAgenda(basicCompleteSlot, {
              ...
              topicDescription:
                "Simulado geral abrangendo todas as disciplinas do concurso. Uma excelente oportunidade de testar seus conhecimentos em um formato similar ao da prova real.",
        ...
        if (pendingTopics.length === 0 && progressPercentage >= 0.95) {
          ...
          while (simuladoCount < maxSimulados) {
            nextMaintenanceDay = findNextAvailableSlot(
              nextMaintenanceDay,
              false
            );
            if (!nextMaintenanceDay) break;

            const simuladoDescription =
            ...
            nextMaintenanceDay.setDate(
              nextMaintenanceDay.getDate() + simuladoFrequency
            );
            simuladoCount++;
          }
        }
      ...
      console.log(
        `[PERF] Creating ${sessionsToCreate.length} sessions in batch`
      );
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const overdueSessions = await dbAll(
      ...
      const sessionCountsQuery = `
            ...
            `;
      const sessionCountsResult = await dbAll(sessionCountsQuery, [
        planId,
        todayStr,
        endDateStr,
      ]);
        ...
        for (const session of sessions) {
          ...
          if (futureSessionsOfSubject.length > 0) {
            const nextSessionDate = new Date(
              futureSessionsOfSubject[0].session_date
            );
            const insertDate = new Date(nextSessionDate);
            insertDate.setDate(insertDate.getDate() - 1);

            const slot = findAvailableSlotPreview(
              insertDate > new Date() ? insertDate : new Date()
            );
      ...
      res.json({
        ...
        description:
          "As tarefas atrasadas serão reagendadas de forma inteligente: preferencialmente antes das próximas sessões da mesma matéria, preservando a continuidade do aprendizado.",
    ...
    try {
      ...
      const findNextAvailableSlot = async (
        ...
        while (currentDate <= examDate && daysSearched < maxDaysSearch) {
          ...
          const currentSessionCountResult = await dbGet(
      ...
      const smartReplan = async () => {
        console.log(
          `[REPLAN] Iniciando replanejamento inteligente para ${overdueSessions.length} sessões atrasadas`
        );
        ...
        const loadSessionsForDate = async (dateStr) => {
          if (!sessionDateCache.has(dateStr)) {
            const sessions = await dbAll(
              "SELECT id, subject_name FROM study_sessions WHERE study_plan_id = ? AND session_date = ?",
              [planId, dateStr]
            );
        ...
        Object.keys(sessionsBySubject).forEach((subject) => {
          sessionsBySubject[subject].sort((a, b) => {
            ...
            return (
              (priorityOrder[a.session_type] || 5) -
              (priorityOrder[b.session_type] || 5)
            );
          });
        });
        ...
        for (const [subject, sessions] of Object.entries(sessionsBySubject)) {
          console.log(
            `[REPLAN] Processando ${sessions.length} sessões da matéria: ${subject}`
          );

          const futureSessionsOfSubject =
            futureSessionsBySubject[subject] || [];
          ...
          for (const session of sessions) {
            ...
            const sessionExists = await dbGet(
              'SELECT id, status FROM study_sessions WHERE id = ? AND status = "Pendente"',
              [session.id]
            );
            if (!sessionExists) {
              console.log(
                `[REPLAN] ⚠ Sessão ${session.id} não existe ou não está pendente - ignorando`
              );
              continue;
            }
            ...
            if (futureSessionsOfSubject.length > 0) {
              ...
              if (slot && slot.date < nextSessionDate) {
                ...
                const sameSubjectCount = sessionsOnDate.filter(
                  (s) => s.subject_name === session.subject_name
                ).length;
                ...
                if (sameSubjectCount < 2) {
                  ...
                  sessionDateCache.get(newDateStr).push({
                    id: session.id,
                  ...
                  rescheduledCount++;
                  reschedulingLog.push(
                    `${session.subject_name}: ${session.topic_description} → ${newDateStr} (${strategy})`
                  );
                  console.log(
                    `[REPLAN] ✓ Sessão ${session.id} reagendada para ${newDateStr} (${strategy})`
                  );
                }
              }
            }
            ...
            if (!rescheduled) {
              ...
              if (lateSlot) {
                ...
                rescheduledCount++;
                reschedulingLog.push(
                  `${session.subject_name}: ${session.topic_description} → ${newDateStr} (${strategy} - ATENÇÃO!)`
                );
                console.log(
                  `[REPLAN] ⚠ Sessão ${session.id} reagendada para ${newDateStr} (${strategy})`
                );
            ...
            if (!rescheduled) {
              ...
              console.log(
                `[REPLAN] ✗ Não foi possível reagendar sessão ${session.id} - sem slots disponíveis`
              );
            }
          }
        }
      ...
      console.log(
        `- Sessions reagendadas: ${result.rescheduledCount}/${overdueSessions.length}`
      );
      console.log(
        `- Sessions não reagendadas: ${result.failedSessions.length}`
      );
      ...
      res.json({
        ...
        details: {
          ...
          successRate: Math.round(
            (result.rescheduledCount / overdueSessions.length) * 100
          ),
      ...
      res.status(500).json({
        success: false,
        error:
          "Ocorreu um erro interno ao replanejar as tarefas. Nossa equipe foi notificada.",
        message:
...
app.get(
  ...
  query("type")
    .isIn(["semanal", "mensal"])
    .withMessage("Tipo de revisão inválido"),
  handleValidationErrors,
  async (req, res) => {
    ...
    try {
      const plan = await dbGet(
        "SELECT review_mode FROM study_plans WHERE id = ?",
        [planId]
      );
      if (!plan) return res.status(404).json({ error: "Plano não encontrado" });
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      const plan = await dbGet(
        "SELECT plan_name, exam_date FROM study_plans WHERE id = ? AND user_id = ?",
        [planId, userId]
      );
      if (!plan)
        return res
      ...
      if (completedSessions.length > 0) {
        ...
        const lastStudyDate = new Date(
          completedSessions[0].session_date + "T00:00:00"
        );
        ...
        if (
          ...
          for (let i = 1; i < completedSessions.length; i++) {
            ...
            const nextStudyDate = new Date(
              completedSessions[i].session_date + "T00:00:00"
            );
            if (nextStudyDate.getTime() === previousDay.getTime()) {
              studyStreak++;
              currentDate = nextStudyDate;
      ...
      const levels = [
        ...
        {
          threshold: 51,
          title: "Mestre dos Grupos de WhatsApp de Concurso 📲",
        },
        {
          threshold: 101,
          title: "Gabaritador(a) da prova de Português da FGV 🎯",
        },
        { threshold: 201, title: "Terror do Cespe/Cebraspe 👹" },
        { threshold: 351, title: "Veterano(a) de 7 Bancas Diferentes 😎" },
        {
          threshold: 501,
          title: "✨ Lenda Viva: Assinante Vitalício do Diário Oficial ✨",
        },
      ];
      ...
      const shareData = {
        ...
        shareText:
          `🎯 MEU PROGRESSO NO ${plan.plan_name.toUpperCase()}!\n\n` +
      ...
      res
        .status(500)
        .json({ error: "Erro ao gerar dados de compartilhamento." });
    }
  }
);
